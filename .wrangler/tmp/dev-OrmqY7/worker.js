...
// .wrangler/tmp/bundle-Ze4uAJ/checked-fetch.js
...
var ALLOWED_ORIGIN = "https://reflectiv-ai.com";
...
async function handleAuthLogout(request) {
  ...
  return setCookie(response, "session", "", 0);
}
...
__name(handleAppSettings, "handleAppSettings");
async function handleLlmInvoke(request, env) {
  const body = await request.json().catch(() => ({}));
  const { prompt, response_json_schema, max_tokens = 2e3, temperature = 0.7, roleplay = false } = body;
  if (!prompt) {
    return new Response(
  ...
  try {
    const messages = roleplay ? [{ role: "system", content: prompt }] : [
...
Always respond with actionable, behavior-specific feedback.${response_json_schema ? "\nFormat your response as valid JSON matching the provided schema." : ""}` },
    ...
    const openaiRequest = {
      ...
      temperature,
      max_tokens,
      response_format: response_json_schema ? { type: "json_object" } : void 0
    };
    Object.keys(openaiRequest).forEach(
      (key) => openaiRequest[key] === void 0 && delete openaiRequest[key]
    );
    const llmUrl = provider === "groq" ? "https://api.groq.com/openai/v1/chat/completions" : "https://api.openai.com/v1/chat/completions";
...
__name(handleUserLogs, "handleUserLogs");
async function handleSnippets(request) {
  ...
  if (search) {
    results = results.filter(
      (s) => s.title.toLowerCase().includes(search) || s.content.toLowerCase().includes(search) || s.tags?.some((tag) => tag.toLowerCase().includes(search))
    );
  }
  return Response.json({
    snippets: results,
...
__name(handleSnippets, "handleSnippets");
async function handleAssignments(request) {
  ...
  if (request.method === "PATCH") {
    ...
    if (assignment) {
      ...
      assignment.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
    }
    return Response.json({ success: true, assignment });
  }
  return Response.json({ error: "Method not allowed" }, { status: 405 });
}
__name(handleAssignments, "handleAssignments");
async function handleLearningPaths(request) {
  ...
  if (request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const { sessions: sessions2 = [] } = body;
    const analyzedPaths = learningPaths.map((path) => ({
      ...path,
      session_count: sessions2.length > 0 ? Math.floor(Math.random() * 10) + 1 : 0,
      avg_score: sessions2.length > 0 ? (Math.random() * 2 + 2.5).toFixed(1) : null
    }));
    return Response.json({ learningPaths: analyzedPaths, analyzed: true });
  }
  return Response.json({ error: "Method not allowed" }, { status: 405 });
}
__name(handleLearningPaths, "handleLearningPaths");
...
__name(handleRolePlaySessions, "handleRolePlaySessions");
async function handleCustomScenarios(request) {
  ...
  if (request.method === "PUT") {
    try {
      ...
      if (idx >= 0) {
        customScenarios[idx] = { ...customScenarios[idx], ...body, updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
...
__name(handleCustomScenarios, "handleCustomScenarios");
...
// .wrangler/tmp/bundle-Ze4uAJ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Ze4uAJ/middleware-loader.entry.ts
