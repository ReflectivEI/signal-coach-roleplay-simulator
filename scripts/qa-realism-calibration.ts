...
test("audit output includes severity and evidence details", () => {
    ...
    assert(Array.isArray(firstFailure?.expected_topic_lanes), "expected_topic_lanes missing");
    assert(Boolean(firstFailure?.matched_topic_lane), "matched_topic_lane missing");
    assert(Array.isArray(firstFailure?.expected_stage_behavior), "expected_stage_behavior missing");
});
