...
test("Predictive Builder context remains the authoritative HCP dialogue route", () => {
  const simulatorSource = fs.readFileSync(
  ...
  assert.match(simulatorSource, /buildPredictivePromptContext\(predictiveRuntimeData\)/);
  assert.match(simulatorSource, /predictiveProfile,\s*\n\s*predictiveContext/);
  assert.match(generatorSource, /PREDICTIVE HCP BRAIN \(AUTHORITATIVE SOURCE OF TRUTH FOR HCP DIALOGUE\)/);
  assert.match(generatorSource, /The Predictive HCP Brain authors the HCP line/);
  assert.match(generatorSource, /predictiveContext: predictivePromptContext/);
  assert.match(generatorSource, /predictive_context_received: Boolean\(predictivePromptContext\.trim\(\)\)/);
  assert.match(generatorSource, /guardrails_demoted_to_validators: true/);
});

test("HCP cue generation blocks repeated active-turn cue overrides", () => {
  ...
  assert.match(cueGeneratorSource, /const recentCueFingerprints = new Set/);
  assert.match(cueGeneratorSource, /const activeTurnCue = buildActiveTurnCueOverride/);
  assert.match(cueGeneratorSource, /const repeatsRecentCue = recentCueFingerprints\.has\(activeFingerprint\)/);
  assert.match(cueGeneratorSource, /activeSignature === lastSignature/);
  assert.match(cueGeneratorSource, /activeFrame === lastFrame/);
  assert.match(cueGeneratorSource, /if \(!repeatsRecentCue\)/);
  assert.match(cueGeneratorSource, /fallbackCueVariants/);
  assert.match(cueGeneratorSource, /recentCueFingerprints\.has\(normalizeCueFingerprint\(derived\)\)/);
  assert.match(generatorSource, /return \[turn\?\.cueBefore, turn\?\.observedCue, \.\.\.surfaced\]\.filter\(Boolean\)/);
  assert.match(generatorSource, /function enforceFinalCueUniqueness/);
  assert.match(generatorSource, /const uniqueCue = enforceFinalCueUniqueness/);
  assert.match(generatorSource, /cue\?\.description/);
  assert.match(generatorSource, /slice\(-30\)/);
});



test("HCP dialogue loop guard rotates repeated direct-answer demands", () => {
  const generatorSource = fs.readFileSync(
    new URL("../src/lib/hcpResponseGenerator.ts", import.meta.url),
    "utf8",
  );

  assert.match(generatorSource, /direct_answer_loop/);
  assert.match(generatorSource, /function enforceFinalHcpReplyUniqueness/);
  assert.match(generatorSource, /Math\.min\(10, limit\)/);
  assert.match(generatorSource, /Name the patient profile first, then the endpoint that would change my decision/);
  assert.match(generatorSource, /I need the safety point tied to a specific patient decision/);
  assert.match(generatorSource, /selectNonRepeatingFallbackVariant\(\{/);
});

test("right-panel recommended intervention recovery responses rotate after repeated use", () => {
  const simulatorStoreSource = fs.readFileSync(
    new URL("../src/lib/simulatorIntelligenceStore.ts", import.meta.url),
    "utf8",
  );
  const nextEventSource = fs.readFileSync(
    new URL("../src/lib/predictiveNextEventEngine.ts", import.meta.url),
    "utf8",
  );

  assert.match(simulatorStoreSource, /const RECOVERY_RESPONSE_VARIANTS = \{/);
  assert.match(simulatorStoreSource, /function countRecentSimilarRepUses/);
  assert.match(simulatorStoreSource, /function selectRecoveryResponse/);
  assert.match(simulatorStoreSource, /selectRecoveryResponse\(input\.turns, "access"\)/);
  assert.match(simulatorStoreSource, /selectRecoveryResponse\(input\.turns, "boundary"\)/);
  assert.match(simulatorStoreSource, /selectRecoveryResponse\(input\.turns, "broad"\)/);
  assert.match(nextEventSource, /const RECOVER_ACCESS_RESPONSES = \[/);
  assert.match(nextEventSource, /function selectRecoveryAccessResponse/);
  assert.match(nextEventSource, /safestResponse: selectRecoveryAccessResponse\(input\)/);
  assert.doesNotMatch(nextEventSource, /safestResponse: RECOVER_ACCESS_RESPONSE/);
});
