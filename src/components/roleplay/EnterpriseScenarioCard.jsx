...
export default function EnterpriseScenarioCard({
  ...
  const [typedScene, setTypedScene] = useState("");
  ...
  const openingScene = String(
    scenario.openingScene
    || scenario.opening_scene
    || "Preview the opening moment to hear how the HCP enters the conversation."
  ).trim();
  ...
  useEffect(() => {
    ...
    previewTimerRef.current = window.setInterval(() => {
      index += 1;
      setTypedScene(openingScene.slice(0, index));
  ...
  }, [expanded, previewing, openingScene]);

  return (
    <div className={`scenario-card flex h-full flex-col self-start overflow-hidden rounded-2xl border bg-white transition-all duration-150 ease-in-out hover:-translate-y-[1px] hover:border-teal-300 hover:shadow-lg ${expanded ? "scenario-card-expanded border-[#1A334D] shadow-xl shadow-teal-100/70" : "border-[#1A334D]/70 shadow-md"}`}>
      <div className={`flex flex-1 flex-col px-5 pt-5 ${expanded ? "pb-4" : "pb-5"}`}>
        <div className="flex items-start gap-2">
          <h3 className="flex-1 font-bold leading-snug text-gray-900 text-sm sm:text-[15px]">{scenario.title}</h3>
          <span className={`flex-shrink-0 rounded-full border px-2.5 py-0.5 text-xs font-semibold ${diff.color}`}>{diff.label}</span>
        </div>
        ...
        {expanded ? (
          <div className="mt-4 space-y-4 border-t border-slate-100 pt-4">
            ...
            {scenario.description && <p className="text-xs leading-relaxed text-gray-600">{scenario.description}</p>}
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-3">
              <div className="mb-2 flex items-center justify-between gap-2">
                ...
                >
                  {previewing ? "Reset Scene" : openingSceneLabel}
                </button>
              </div>
              <div className="min-h-[72px] rounded-lg border border-slate-200 bg-white/80 px-3 py-2.5">
                <p className={`text-xs leading-relaxed text-slate-600 ${previewing ? "" : "text-slate-400"}`}>
                  {previewing ? typedScene || " " : "Press Play Scene to reveal the opening beat."}
                </p>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {tacticalFocusLines.map((item, index) => (
                    <span key={index} className="max-w-full break-words rounded bg-gray-100 px-2 py-1 text-xs text-gray-700">{item}</span>
                  ))}
                </div>
              </div>
            )}
      ...
      <div className={`flex flex-wrap items-center gap-3 px-5 pb-5 ${expanded ? "pt-1" : "pt-0"}`}>
        <button
          type="button"
