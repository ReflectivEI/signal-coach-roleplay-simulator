...
import { BookOpen, Brain, ChevronDown, FlaskConical, GitBranch, Menu, Settings, X, Zap } from "lucide-react";
...
function DesktopAdminMenu() {
  return (
    <div className="group relative">
      ...
      <div className="invisible absolute right-0 top-full z-30 min-w-[168px] pt-2 opacity-0 transition-all duration-150 group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
        <div
        ...
        >
          {ADMIN_NAV_ITEMS.map(({ to, label, Icon }) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) => [
                "flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors hover:text-teal-700",
                isActive ? "text-teal-700" : "text-[#17213f]",
              ].join(" ")}
            >
              <Icon className="w-3.5 h-3.5" />
        ...
        {menuOpen && (
          <nav id={menuId} className="xl:hidden pt-3" aria-label="Mobile primary">
            ...
            >
              ...
              <div
                className="rounded-2xl border bg-white/55 p-2"
                style={{
                  borderColor: "rgba(92, 135, 165, 0.22)",
                }}
              >
                <p className="px-2 pb-1 text-[11px] font-semibold uppercase tracking-[0.16em]" style={{ color: "hsl(222 40% 30%)" }}>
                  Admin
                </p>
