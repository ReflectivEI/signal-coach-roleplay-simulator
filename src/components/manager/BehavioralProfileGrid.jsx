  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
      <div className="flex flex-col gap-2 border-b border-slate-200 pb-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <span className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold text-slate-600">Canonical order · 8 capabilities</span>
      </div>

      <div className={`mt-4 grid gap-3 ${compact ? "sm:grid-cols-2 xl:grid-cols-1" : "sm:grid-cols-2 xl:grid-cols-4"}`}>
        {items.map((item) => {
          ...
          return (
            <div key={item.key} className={`rounded-xl border px-3 py-3 ${getTone({ isStrongest, isWeakest, isBelowThreshold })}`}>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {isStrongest ? <span className="rounded-full bg-white/80 px-2 py-1 text-[10px] font-semibold text-teal-700">Strongest</span> : null}
                {isWeakest ? <span className="rounded-full bg-white/80 px-2 py-1 text-[10px] font-semibold text-amber-700">Weakest</span> : null}
                {!isWeakest && !isStrongest && isBelowThreshold ? <span className="rounded-full bg-white/80 px-2 py-1 text-[10px] font-semibold text-amber-700">Below 3.5/5 threshold</span> : null}
              </div>
            </div>
          );
        })}
