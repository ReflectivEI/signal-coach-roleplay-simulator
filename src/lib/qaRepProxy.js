...
export function buildDeterministicQaRepReply({ turns = [], draft = "" }) {
  ...
  return finalizeRepReply(
    pickProgressionLine(mode, topic, previousRepText, lastHcpMessage, turns),
    { hcpTurn: lastHcpMessage, transcript: turns },
  );
}
...
export function enforceRepAnswerFirstContract({
  ...
  const questionType = detectHcpQuestionType(lastHcpMessage);
  if (questionType !== "solution_seeking") {
    return finalizeRepReply(String(draft || "").trim(), { hcpTurn: lastHcpMessage, transcript: turns });
  }

  const normalizedDraft = String(draft || "").trim();
  if (!normalizedDraft) {
    return finalizeRepReply(buildSolutionSeekingFallback({ scenario, turns }), {
  ...
  if (!draftIsQuestion && fullyAnswersQuestion(lastHcpMessage, normalizedDraft)) {
    return finalizeRepReply(normalizedDraft, { hcpTurn: lastHcpMessage, transcript: turns });
  }
