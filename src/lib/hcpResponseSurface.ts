...
function softenHostileTone(text = ""): string {
  return normalizeText(text)
    .replace(/\bI'm intrigued by (?:the )?(?:new )?study, but\b[^.?!]*[.?!]?/gi, "The study could be useful. How does it apply to my patients here?")
    ...
    .replace(/\bworth diverting my limited time to discuss it further\b/gi, "worth discussing now")
    .replace(/\bdiverting attention from\b/gi, "moving away from")
    .replace(/\bgiven my already packed schedule\b/gi, "with the time I have")
    .replace(/\bnot a lot of time to spare\b/gi, "only a minute")
