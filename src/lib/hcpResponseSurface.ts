...
function softenProfessionalEntryTone(
  text: string,
  turn: HcpTurnDirectiveSet,
  scenario: any,
  hcpTurnCount = 0,
  ...
  output = output.replace(/^Look,\s+/i, "");

  if (turn.phase === "initial_access" && turn.concernFamily === "time") {
    output = output
      .replace(
        /^I've got four minutes before my next patient\.\s+My MA said you had something about prior auth reduction, that's why I said yes\.?$/i,
        "My MA said this was about prior auth reduction. I've only got a couple minutes, so tell me what you can actually do to make that easier on my staff."
      )
      .replace(
        /^I've got a few minutes before my next patient\.\s+What(?:'s| is) this about, can you make it quick\??$/i,
        "I've only got a minute, so give me the short version of why this matters right now."
      )
      .replace(/\bcan you make it quick\b/gi, "give me the short version of why this matters right now")
      .replace(/\bmake it quick\b/gi, "give me the short version of why this matters right now");
  }

  if (
    turn.phase === "initial_access" &&
    Array.isArray(scenario?.interactionPressure) &&
    scenario.interactionPressure.includes("operationally_constrained")
  ) {
    output = output.replace(
      /^I've got a few minutes\.\s+My office manager said you've been trying to get in\.\s+Keep it useful and get to the point\.?$/i,
      "My office manager said you've been trying to get in. I've got a few minutes, so tell me why I should make time for this."
    );
  }
...
function normalizeAccessLanguage(text = ""): string {
  let t = normalizeText(text);
  if (!t) return t;

  t = t.replace(
    /I don'?t know what I can actually do about(?: that)?\.?/i,
    "I'm not sure what options I actually have there."
  );

  t = t.replace(
    /formulary team flagged it as non-preferred/gi,
    "our formulary team has it listed as non-preferred right now"
  );

  t = t.replace(/\bour our formulary team\b/gi, "our formulary team");

  if (/non-preferred/i.test(t) && !/\bprocess\b/i.test(t) && !/extra steps on our end/i.test(t)) {
    t = `${t.replace(/[.?!]*$/, ".")} That usually turns into a lot of extra steps on our end.`;
  }

  return normalizeText(t);
}

function completeTrailingClause(text = ""): string {
  let output = normalizeText(text);
  if (!output) return output;

  output = output
    .replace(/\babout\.?$/i, "about that.")
    .replace(/\bwith\.?$/i, "with that process.")
    .replace(/\bfor\.?$/i, "for my patients.");
...
export function applyHcpResponseSurface({
  ...
  output = softenProfessionalEntryTone(output, turn, scenario, hcpTurnCount);
  output = normalizeAccessLanguage(output);
  ...
  return ensureTerminalPunctuation(
    repairQuestionPunctuation(
      repairIncompleteOperationalAsks(
        normalizeAccessLanguage(
          completeTrailingClause(
            repairDanglingTail(
              enforceSentenceCase(
