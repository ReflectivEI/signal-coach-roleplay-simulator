...
function buildManagerInsightsPrompt(payload, derived) {
    return [
        ...
        "- Treat the provided data as factual and correct",
        "- NEVER invent, infer, rename, or substitute values not present",
        "- NEVER mismatch capability names and scores",
        "- ALWAYS reference the rep by name when the insight is rep-specific",
        "- ALWAYS align insights with BOTH rep-level and territory-level data",
        "- Generate recommendations only when they are supported by the data",
        "",
        "Rep Data:",
        ...
        "2. If referencing a score, use the EXACT score tied to that capability.",
        ...
        "5. Recommendations must be specific, behavior-based, and non-generic.",
        "6. Do not output advice that cannot be traced back to provided data.",
        "7. Predictive outlook must reference engagement, performance trend, and capability gaps.",
        "8. Risk items must include a root cause tied to engagement, capability gaps, territory conditions, or sales trend.",
        "9. Reject output if any capability mismatch exists, if any score is assigned to the wrong capability, if the weak area conflicts with the canonical dataset, or if generic language appears.",
        "",
        "Recommendation format:",
        "Action → Situation → Expected Outcome",
        JSON.stringify({
            action: "Run 2 targeted coaching sessions this week focused on commitmentGeneration in oncology account conversations",
            rationale: "Alex Thompson's lowest behavioral metric is commitmentGeneration, and this aligns with lower conversion behavior despite otherwise stable signalAwareness.",
            expectedImpact: "Improved close quality and stronger next-step commitment capture in field conversations",
        }),
        "",
